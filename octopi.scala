...
object OctoTree {
	...
	def tensor_temps(e : Exp) : Set[Tensor] = e match {
		case Tensor(n,i) => Set(Tensor(n,i))
		case Mul(res, lhs, rhs) => tensor_temps(lhs).union(tensor_temps(rhs)) // + res not storing in mults for now
		case Sum(res, i, rhs) => tensor_temps(rhs) + res
	}
